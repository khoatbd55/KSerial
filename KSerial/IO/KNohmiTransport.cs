...
namespace KSerial.IO
{
    public class KNohmiTransport
    {
        // Delegate for MessageModbus Event Handle
        public delegate void MessageRecieveEventHandle(object sender, KSerialTransport_EventArgs e);
        ...
        private void OnMessageRecieve(KSerialTransport_EventArgs msg)
        {
                    ...
                    try
                    {
                        ...
                        if (frame[0] == '\r')
                        {
                            ...
                            if (frame2[0] == '\n')// kết thúc chuỗi
                            {
                                if (listByte.Count > 2)
                                {
                                    OnMessageRecieve(new KSerialTransport_EventArgs()
                                    {
